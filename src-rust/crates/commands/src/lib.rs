...
impl SlashCommand for BudgetCommand {
    ...
    fn description(&self) -> &str {
        "Show or change the output token budget per response"
    }
    fn help(&self) -> &str {
        ...
        if args.is_empty() {
            ...
            if capped != tokens {
                ...
                CommandResult::ConfigChangeMessage(
                    new_config,
                    format!(
                        "Budget capped to {} tokens (hard limit: {})",
                        capped,
                        claurst_core::constants::MAX_TOKENS_HARD_LIMIT
                    ),
                )
            } else {
                let mut new_config = ctx.config.clone();
...
impl SlashCommand for InitCommand {
    ...
    fn description(&self) -> &str {
        "Deep recon scan: map project state and produce ATTRACTOR.md + STATE.md"
    }
    fn help(&self) -> &str {
        "Usage: /init\n\n\
